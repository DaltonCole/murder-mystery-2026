...
* Instead of my current style, I am thinking of converting the style to more of a blood on the clocktower style where the group decides on a single person to eliminate every round (or 2 if we have enough players). The goal of the Aristocrats would then be to remove the revolutionary leader while the goal of the revolutionists would be to survive the night. If the revolutionaist leader is eliminated too early, then their title would be passed onto another revolutionist. The cultist party would act as a balancing party.
    ...
    * Story element:
        * The revolutionaries are trying to influence The Ton by trying to survive the night. The Ton (Aristocrats) are trying to uncover who the revolutionary leader is. The cult leader has two possible win conditions 1) convert both the king/queen and revolutionary leader or 2) convert the king/queen or revolutionary leader AND be sentenced to death by the masses (i.e. die by popular vote - the story element for this is the leader they corrupted is so heartbroken by the cult leader's death that they are galvanized to perform the cult's mission)
