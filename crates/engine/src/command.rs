...
pub enum Command {
    ...
    /// Out more than one person). `actual_winner` is the one faction that
    /// actually won -- `win_condition::evaluate` guarantees exactly one
    /// (Dalton's ruling: only one faction ever wins, and the Cult has
    /// priority over any overlap with Ton's or the Uprising's own
    /// condition). Once per game.
